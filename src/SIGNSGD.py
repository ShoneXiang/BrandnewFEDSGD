...
def SIGNSGD(args, test_dataset, train_dataset, user_groups, N_us, file_path, transmit_power, bitwidths, prune_rates, computing_resources, I_us, h_us):
    ...
    T_SIGNSGD = fl_utils.calculate_T(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.s)
    E_SIGNSGD, T_1max, T_1min, T_2max, T_2min, E_1max, E_1min, E_2max, E_2min  = fl_utils.calculate_E(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.k,args.sigma)
    dataamount_SIGNSGD = fl_utils.calculate_dataamount(bitwidths,prune_rates,args.V,xis)
    ...
    file_name2 = f'TE_SIGNSGD_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
    ...
    datatosave = {'losses_train':losses_train_SIGNSGD, 'accuracies_train':accuracies_train_SIGNSGD, 'accuracies_test':accuracies_test_SIGNSGD}
    file_name1 = f'LA_SIGNSGD_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
    fl_utils.Save_to_Csv(data = datatosave, file_name = file_name1, Save_format = 'csv',Save_type = 'row', file_path=file_path)

    T_SIGNSGD_step = [T_SIGNSGD*step1,T_SIGNSGD*step2,T_SIGNSGD*step3]
    E_SIGNSGD_step = [E_SIGNSGD*step1,E_SIGNSGD*step2,E_SIGNSGD*step3]
    datatosave = {'T_step':T_SIGNSGD_step, 'E_step':E_SIGNSGD_step, 'step':[step1,step2,step3]}
    file_name3 = f'TE_SIGNSGD_step_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
