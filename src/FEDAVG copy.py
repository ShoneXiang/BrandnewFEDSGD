...
class Server:
    def __init__(self, model, test_dataset, optimizer,device,learning_rate):
        self.model = model
...
def FEDAVG(args, test_dataset, train_dataset, user_groups, N_us, file_path, transmit_power, bitwidths, prune_rates, computing_resources, I_us, h_us):
    ...
    server = Server(model=global_model, test_dataset=test_dataset, optimizer=optimizer, device=device, learning_rate=args.learning_rate)
    # 创建客户端
