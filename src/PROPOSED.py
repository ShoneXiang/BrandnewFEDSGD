...
def PROPOSED(args, test_dataset, train_dataset, user_groups, N_us, file_path, transmit_power, bitwidths, prune_rates, computing_resources, I_us, h_us):
    ...
    # client_gmaxs = [torch.tensor(0.0057), torch.tensor(0.0092), torch.tensor(0.0047), torch.tensor(0.0048), torch.tensor(0.0050), torch.tensor(0.0050), torch.tensor(0.0049), torch.tensor(0.0049), torch.tensor(0.0062), torch.tensor(0.0048),torch.tensor(0.0057), torch.tensor(0.0092), torch.tensor(0.0047), torch.tensor(0.0048), torch.tensor(0.0050)]
    # client_gmins = [torch.tensor(-0.0069), torch.tensor(-0.0075), torch.tensor(-0.0071), torch.tensor(-0.0071), torch.tensor(-0.0070), torch.tensor(-0.0071), torch.tensor(-0.0071), torch.tensor(-0.0069), torch.tensor(-0.0070), torch.tensor(-0.0072), torch.tensor(-0.0075), torch.tensor(-0.0071), torch.tensor(-0.0071), torch.tensor(-0.0070), torch.tensor(-0.0071)]
    # client_gmaxs = client_gmaxs[:args.num_clients]
    # client_gmins = client_gmins[:args.num_clients]
    ...
    # 用提出方法调整超参数
    transmit_power,bitwidths,prune_rates = fl_utils.adjust(bitwidths,prune_rates,transmit_power,I_us,h_us,client_gmaxs, client_gmins, computing_resources, np.array(xis), args.num_clients, args.power_min, args.power_max, args.bitwidth_min, args.bitwidth_max, args.prune_rate_min, args.prune_rate_max, args.bcd_epoch, args.BO_epoch, args.N0, args.V, args.c0, args.s, args.k, args.sigma, args.Emax, args.Tmax, N_us, args.B_u, args.waterfall_thre, args.L, args.D, client_gmaxs,client_gmins, args.acq_func)

    T_PROPOSED = fl_utils.calculate_T(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.s)
    E_PROPOSED, T_1max, T_1min, T_2max, T_2min, E_1max, E_1min, E_2max, E_2min  = fl_utils.calculate_E(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.k,args.sigma)
    dataamount_PROPOSED = fl_utils.calculate_dataamount(bitwidths,prune_rates,args.V,xis)
    ...
    file_name2 = f'TE_PROPOSED_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
    ...
    # 打开一个文件用于写入
    with open(f'array_proposed_step_T{args.Tmax}_E{args.Emax}_w{args.wer}.txt', 'w') as f:
        # 写入数组字符串
        f.write('power:'+array_power+'\n')
        f.write('bitwidths:'+array_bitwidths+'\n')
        f.write('prune_rates:'+array_prune_rates+'\n')
        if np.any(fl_utils.constraint_E_forBO(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.k,args.sigma,args.Emax)>0) or np.any(fl_utils.constraint_T_forBO(transmit_power,bitwidths,prune_rates,computing_resources,N_us,args.B_u,h_us,args.N0,I_us,args.V,xis,args.c0,args.s,args.Tmax)>0):
            f.write('不是可行解')
        else:
            f.write('是可行解')
    ...
    datatosave = {'losses_train':losses_train_PROPOSED, 'accuracies_train':accuracies_train_PROPOSED, 'accuracies_test':accuracies_test_PROPOSED}
    file_name1 = f'LA_PROPOSED_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
    fl_utils.Save_to_Csv(data = datatosave, file_name = file_name1, Save_format = 'csv',Save_type = 'row', file_path=file_path)

    T_PROPOSED_step = [T_PROPOSED*step1,T_PROPOSED*step2,T_PROPOSED*step3]
    E_PROPOSED_step = [E_PROPOSED*step1,E_PROPOSED*step2,E_PROPOSED*step3]
    datatosave = {'T_step':T_PROPOSED_step, 'E_step':E_PROPOSED_step, 'step':[step1,step2,step3]}
    file_name3 = f'TE_PROPOSED_step_T{args.Tmax}_E{args.Emax}_w{args.wer}_c{args.count_py}'
